...
def get_resource_path(filename):    # Gets the correct path for a resource file whether running as script or frozen exe
...
def age_adj(age_categ):                                         # accepts age category ('young adult')
    ...
    # with open('agecategories.csv') as bonuses:       <--- this was the previous format for this, pre pyinstaller
    with open(get_resource_path('agecategories.csv')) as bonuses:
...
def age_cat(race, age):                                         # accepts ('High Elf', 452)    STR / INT
    ...
    with open(get_resource_path('attrbonuses.csv')) as age_categories:
...
def racialsum(race):                                            # accepts ('High Elf')
    ...
    with open(get_resource_path('attrbonuses.csv')) as racial_bonuses:
        for row in csv.reader(racial_bonuses):                  # ie, high elf atts are +2 (net) vs a human's
            if race == row[0]:
                for a in range(1, 8):
...
def exceptional_str(race):                                          # accepts ('High Elf')
    with open(get_resource_path('attrbonuses.csv')) as bonuses:     # returns (75)
...
def racial_attribute_bonus(race):                               # accepts ('High Elf')
    ...
    with open(get_resource_path('attrbonuses.csv')) as values:
...
def racial_maximums(race):                                      # accepts ('High Elf')
    ...
    with open(get_resource_path('attributemax.csv')) as maxs:
...
def age_variables(race, ch_class):          # returns a three-item list [base_age, #ofrolls, #ofsides]
    ...
    with open(get_resource_path('attributemins.csv')) as minput:
...
def minimum_sum(ch_class):                                      # accepts ('Ranger')
    classminimums, tempsum = open(get_resource_path('attributemins.csv')), 0       # returns (67)
...
def minimums(cla_rac):                                          # accepts ('High Elf')
    ...
    with open(get_resource_path('attributemins.csv')) as mins:  # handles both race and class arguments
...
def class_ac(ch_classes, levels):                       # accepts (('Kensai'), (5))
    ...
    for index, (ch_class, level) in enumerate(zip(ch_classes, levels)):
        with open(get_resource_path('attributemins.csv')) as ac_trans:
            for row in csv.reader(ac_trans):            # ...2 = Monk, 1 = Kensai, 0 = all other classes...
                if ch_class == row[0]:
                    result.append(int(row[76]))
        with open(get_resource_path('levelvalues.csv')) as ac_classmods:
            for row_lvl in csv.reader(ac_classmods):    # ...as well as a level modifier (based on transposed column)
...
def dex_acmultiplier(ch_classes):                       # accepts (('Monk'))
    ...
    with open(get_resource_path('xpvalues.csv')) as character_classes:
        for row in csv.reader(character_classes):       # the call filters non-Monks/non-Kensai
...
def dex_acbonus(dexterity):                             # accepts 17
    dex_ac = 0                                          # returns 3
    with open(get_resource_path('attributevalues.csv')) as dexbonus:
...
def race_class_movement(race, ch_classes):              # accepts ('Drow', tuple(['Fighter'])
    ...
    with open(get_resource_path('attributemins.csv')) as movement_rates:
...
def class_level_movement(levels, elig):                 # calculates movement for non-armored characters
    ...
    for index, (transposer, level) in enumerate(zip(elig, levels)):
        with open(get_resource_path('levelvalues.csv')) as move_mods:
...
def comeliness_bonus(charisma):                         # applies charisma bonus to comeliness
    final = 0                                           # accepts 16
    with open(get_resource_path('attributevalues.csv')) as bonuses:        # returns 2
        for row in csv.reader(bonuses):
...
def base_thaco(classes, levels):                        # accepts (tuple['Assassin', 'Fighter'], tuple([4, 4])
    final = []                                          # returns 2
    for ch_class, level in zip(classes, levels):        # first we request a 'transposition' value by archetype
        with open(get_resource_path('xpvalues.csv')) as th_trans:
            for row in csv.reader(th_trans):
                if ch_class == row[0]:                  # 0 for F, 1 for C, 2 for T, 3 for M
                    result = int(row[51])
        with open(get_resource_path('levelvalues.csv')) as thaco_mod:
...
def str_thacobonus(strength, exceptional, str_disp, multiplier):
    ...
    if len(str_disp) > 2:                               # checks for exceptional str
        with open(get_resource_path('excstr.csv')) as excbonus:
        ...
        with open(get_resource_path('attributevalues.csv')) as strbonus:
...
def con_hpbonus(constitution):                          # accepts 17
    with open(get_resource_path('attributevalues.csv')) as attr_bonuses:   # returns 3
...
def demote_class(cha_class):                            # accepts 'Fighter'
    with open(get_resource_path('xpvalues.csv')) as characterclasses:      # returns '0-level'
...
def ua_attr(ch_class):                                  # accepts 'Fighter'
    ...
    with open(get_resource_path('xpvalues.csv')) as ua_dice:
...
def return_xp(ch_class):
    base = []
    with open(get_resource_path('xpvalues.csv')) as currentxp:
...
def xp_bonus_check(cha_class):                  # accepts 'Fighter'
    ...
    with open(get_resource_path('xpvalues.csv')) as maxxp:
...
def call_height_weight(race):
    result = []
    with open(get_resource_path('attrbonuses.csv')) as sizevalues:
